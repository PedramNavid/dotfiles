...
local mpd = lain.widget.mpd({
    settings = function()
        if mpd_now.state == "play" then
            ...
            widget:set_markup(markup.font(beautiful.font, markup(beautiful.black, artist) .. " " .. markup(beautiful.black, title)))
...
widgets.mpd_widget = wibox.container.background(
    wibox.container.margin( wibox.widget { mpd_icon, mpd, layout=wibox.layout.align.horizontal },
                    dpi(3), dpi(6)), beautiful.muted_purple)

widgets.mem_widget =  wibox.container.background( wibox.container.margin( wibox.widget { mem_icon, mem, layout = wibox.layout.align.horizontal },
