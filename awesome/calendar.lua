...
local function worker(user_args)
  ...
  local function decorate_cell(widget, flag, date)
    ...
    if flag == 'focus' then
      ...
      if not (today.month == date.month and today.year == date.year) then flag = 'normal' end
    end
  ...
  function calendar_widget.toggle()
    if popup.visible then
      ...
      elseif placement == 'bottom_left' then
        awful.placement.bottom_left(popup, { margins = { bottom = 30, left = 10 }, parent = awful.screen.focused() })
      else
        awful.placement.top(popup, { margins = { top = 50 }, parent = awful.screen.focused() })
      end
