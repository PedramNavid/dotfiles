...
local function worker(user_args)
		...
		if current_number_of_reviews > previous_number_of_reviews then
			...
			naughty.notify({
				...
				text = reviews[1].project
					...
					.. "\n",
				run = function()
					spawn.with_shell("xdg-open https://" .. host .. "/" .. reviews[1]._number)
				end,
			})
		end
	...
	gerrit_widget:buttons(awful.util.table.join(awful.button({}, 1, function()
...
return setmetatable(gerrit_widget, {
	__call = function(_, ...)
		return worker(...)
	end,
})
