...
local function worker(user_args)
	...
	local update_widget = function(widget, stdout, stderr, _, _)
		...
		for _, pr in ipairs(prs) do
			...
			if not gfs.file_readable(path_to_avatar) then
				spawn.easy_async(string.format(DOWNLOAD_AVATAR_CMD, pr.user.id, pr.user.avatar_url), function()
	...
	github_widget:buttons(awful.util.table.join(awful.button({}, 1, function()
	...
	watch(string.format(GET_PRS_CMD, reviewer), timeout, update_widget, github_widget)
