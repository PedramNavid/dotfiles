...
local function count_approves(participants)
	...
	for i = 1, #participants do
		if participants[i]["approved"] then
			res = res + 1
		end
	end
	return res
...
local function worker(user_args)
	...
	local update_widget = function(widget, stdout, stderr, _, _)
		...
		for i = 0, #to_review_rows do
			to_review_rows[i] = nil
		end
		...
		for i = 0, #my_review_rows do
			my_review_rows[i] = nil
		end
		...
		for _, pr in ipairs(result) do
			...
			row:get_children_by_id("title")[1]:buttons(awful.util.table.join(awful.button({}, 1, function()
				spawn.with_shell("xdg-open " .. pr.links.html.href)
				popup.visible = false
			end)))
			row:get_children_by_id("avatar")[1]:buttons(awful.util.table.join(awful.button({}, 1, function()
				spawn.with_shell(
					string.format(
						'xdg-open "https://bitbucket.org/%s/%s/pull-requests?state=OPEN&author=%s"',
						workspace,
						repo_slug,
						pr.author.uuid
					)
	...
	bitbucket_widget:buttons(awful.util.table.join(awful.button({}, 1, function()
	...
	watch(string.format(GET_PRS_CMD, host, workspace, repo_slug, uuid, uuid), timeout, update_widget, bitbucket_widget)
	return bitbucket_widget
end

return setmetatable(bitbucket_widget, {
	__call = function(_, ...)
		return worker(...)
	end,
})
