...
local function worker(user_args)
	...
	local update_widget = function(widget, stdout, stderr, _, _)
		...
		for _, pr in ipairs(result) do
			...
			if not gfs.file_readable(path_to_avatar) then
				spawn.easy_async(
					string.format(DOWNLOAD_AVATAR_CMD, HOME_DIR, pr.author.id, pr.author.avatar_url),
					function()
	...
	gitlab_widget:buttons(awful.util.table.join(awful.button({}, 1, function()
	...
	watch(
		string.format(GET_PRS_CMD, access_token, host),
...
return setmetatable(gitlab_widget, {
	__call = function(_, ...)
		return worker(...)
	end,
})
