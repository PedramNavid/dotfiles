...
local function worker(user_args)
	...
	local play_pause_btn = awesomebuttons.with_icon({
		type = "outline",
		icon = "play",
		icon_size = 32,
		icon_margin = 8,
	...
	local update_widget = function(widget, stdout, stderr, _, _)
		...
		if (art_url ~= nil or art_url ~= "") and not gfs.file_readable("/tmp/" .. track_id) then
			spawn.easy_async("touch /tmp/" .. track_id, function()
				spawn.easy_async(
					"curl -L -s --show-error --create-dirs -o /tmp/" .. track_id .. " " .. art_url,
	...
	spotify_player.widget:buttons(awful.util.table.join(awful.button({}, 1, function()
		spotify_player:tog()
	end)))
...
return setmetatable(spotify_player, {
	__call = function(_, ...)
		return worker(...)
	end,
})
