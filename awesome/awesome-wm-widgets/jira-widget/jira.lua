...
local function worker(user_args)
	...
	local icon = args.icon
		or HOME_DIR .. "/.config/awesome/awesome-wm-widgets/jira-widget/icon/jira-mark-gradient-blue.svg"
	...
	local update_widget = function(widget, stdout, stderr, _, _)
		...
		for _, issue in ipairs(result.issues) do
			...
			if not gfs.file_readable(path_to_avatar) then
				spawn.easy_async(
					string.format(DOWNLOAD_AVATAR_CMD, HOME_DIR, name, issue.fields.assignee.avatarUrls["48x48"])
				)
			end
	...
	jira_widget:buttons(awful.util.table.join(awful.button({}, 1, function()
